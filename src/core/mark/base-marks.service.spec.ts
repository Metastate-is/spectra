...
class TestMarkService extends BaseMarkService<IOffchainMark> {
  ...
  public async findOnePublic(mark: IOffchainMark, tx: TransactionPromise): Promise<Record<string, any> | null> {
    return this.findOne(mark, tx);
  }

  public async createParticipantIfNotExistsPublic(participantId: string, tx: TransactionPromise): Promise<void> {
